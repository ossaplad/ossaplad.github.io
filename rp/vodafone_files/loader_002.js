(function(exports) {
'use strict';
function DCP1585xClient() {};
// exports.DCP1585xClient = DCP1585xClient;
...
DCP1585xClient.prototype = {
  DCP1585Utils: {
    campaignOpts: {
      ...
      agentId: 'DCP-1585-a', // campaign ID. ESSENTIAL!!!
    ...
    setReactState: function(el, key, value){
      DCP1585xClient.prototype.DCP1585Utils.findReact(el).setState({key: value});
    },
    getReactProps: function(el, key){
      return DCP1585xClient.prototype.DCP1585Utils.findReact(el)[key];
    },
    geoLocation: {},
    getGeo: function(){
      if (!navigator.geolocation) {
        ...
        navigator.geolocation.getCurrentPosition( function(pos){
             DCP1585xClient.prototype.DCP1585Utils.geoLocation = pos;
          },
          function(err){
            DCP1585xClient.prototype.DCP1585Utils.geoLocation = err;
          },
          {
            // timeout: 5000,
            // maximumAge: 0,
    ...
    campaignPath: function(){
      ...
      if(DCP1585xClient.prototype.DCP1585Utils.campaignOpts.planner.time !== undefined){
        if(loaderNow > new Date(DCP1585xClient.prototype.DCP1585Utils.campaignOpts.planner.time).getTime()){
          path = DCP1585xClient.prototype.DCP1585Utils.campaignOpts.planner.path;
        }
      }
      return path;
    },
    getCurrentScriptParameter: function(key){
      return new URL(DCP1585xClient.prototype.DCP1585Utils.currentScriptSrc).searchParams.get(key);
    },
    tracking: function() {},
    isLocalAssetsExist: function(){
      var hasLocalIndex = localStorage.getItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_html') !== null;
      var validTimeStamp = loaderNow <= parseInt(localStorage.getItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_timestamp'));
      if (hasLocalIndex && validTimeStamp && DCP1585xClient.prototype.DCP1585Utils.campaignOpts.saveToLocal){
        return true;
      }
      else{
        localStorage.removeItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_html');
        localStorage.removeItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_timestamp');
        return false;
      }
    },
    appendScript: function(location, script, inlineScript, isAsync, aid) {
      ...
      js.async = isAsync;
      if(script !== null){
        js.src = script[1].indexOf('//') > -1 ? script[1] : DCP1585xClient.prototype.DCP1585Utils.campaignPath() + aid + '/' + script[1];
      }
    ...
    appendStyle: function(location, style, aid) {
      ...
      link.rel = 'stylesheet';
      link.href = style.indexOf('//') > -1 ? style : DCP1585xClient.prototype.DCP1585Utils.campaignPath() + aid + '/' + style;
    ...
    filterScriptStyle: function(location, response, aid) {
      ...
      while ((script = re.exec(response))) {
        ...
        DCP1585xClient.prototype.DCP1585Utils.appendScript(location, src, inlineSrc, isAsync, aid);
      }
      ...
      while ((style = reSty.exec(response))) {
        ...
        if (href !== null) {
          DCP1585xClient.prototype.DCP1585Utils.appendStyle(location, href[1], aid);
        }
  ...
  DCP1585loaderClient: {
    init: function(campaignOpts) {
      ...
      window.location.href.replace(location.hash, '').replace(/[?&]+([^=&]+)=?([^&]*)?/gi, // regexp
        ...
        DCP1585xClient.prototype.DCP1585Utils.vars[key] = value !== undefined ? value : '';
      });
      var pg = 'local';
      if (window.VHA_A) {
        pg = window.VHA_A.page;
      }
      if(DCP1585xClient.prototype.DCP1585Utils.vars.t_version){
        aid = aid + '-' + DCP1585xClient.prototype.DCP1585Utils.vars['t_version'];
      }
      var url = DCP1585xClient.prototype.DCP1585Utils.campaignPath() + aid + '/index.html';

      var isBlocked = false;
      DCP1585xClient.prototype.DCP1585Utils.campaignOpts.excludeDomain.forEach(function(ele, ind, arr){
      ...
      if (DCP1585xClient.prototype.DCP1585Utils.vars.t_loader === '0' || DCP1585xClient.prototype.DCP1585Utils.vars.t_loader === 'false') {
        if (DCP1585xClient.prototype.DCP1585Utils.vars.t_campaign) {
          aid = DCP1585xClient.prototype.DCP1585Utils.vars.t_campaign;
          url = DCP1585xClient.prototype.DCP1585Utils.campaignPath() + aid + '/index.html';
      ...
      if (campaignType === 'demo' && DCP1585xClient.prototype.DCP1585Utils.vars.t_jailbreak !== '1') {
        return false;
      }

      //check if local storage has valid assets
      if(DCP1585xClient.prototype.DCP1585Utils.isLocalAssetsExist()){
        var selector = document.querySelector(sid);
        var CROWDDiv = document.createElement('div');
        var response = localStorage.getItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_html');
        var tidyCode = '';
        ...
        if (response !== null) {
          ...
          tidyCode = DCP1585xClient.prototype.DCP1585Utils.tidyCode(selector, response, aid);
          ...
          DCP1585xClient.prototype.DCP1585Utils.filterScriptStyle(selector, response, aid);

          var t1 = performance.now(); // Loader work has done
          var loaderWorkingTime = parseInt(t1 - t0);
          if (DCP1585xClient.prototype.DCP1585Utils.vars.t_debug === '1' || DCP1585xClient.prototype.DCP1585Utils.vars.t_debug === 'true') {
        ...
        .done(function(response){
          ...
          if(window.localStorage && DCP1585xClient.prototype.DCP1585Utils.campaignOpts.saveToLocal){
            localStorage.setItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_html', response);
            if(DCP1585xClient.prototype.DCP1585Utils.campaignOpts.expireDate !== ''){
              localStorage.setItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_timestamp', new Date(DCP1585xClient.prototype.DCP1585Utils.campaignOpts.expireDate).getTime());
            }
            else{
              localStorage.setItem('__'+DCP1585xClient.prototype.DCP1585Utils.campaignOpts.agentId +'_timestamp', loaderNow + DCP1585xClient.prototype.DCP1585Utils.campaignOpts.lifeCycle);
            }
          }
          var selector = document.querySelector(sid);
          var CROWDDiv = document.createElement('div');
...
// DCP1585xClient.prototype.DCP1585Utils.getGeo(); //get Geolocation for campaign
// make sure selector exists
var DCP1585max = 10;
var DCP1585timerHolder = setInterval(function () {
    var selector = document.querySelector(DCP1585xClient.prototype.DCP1585Utils.campaignOpts.selector);
    if(selector){
      clearInterval(DCP1585timerHolder);
      DCP1585timerHolder = null;
      DCP1585xClient.prototype.DCP1585loaderClient.init(DCP1585xClient.prototype.DCP1585Utils.campaignOpts);
    }
    else{
      DCP1585max --;
